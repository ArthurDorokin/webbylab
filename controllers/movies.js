                    ...
                    for (let i = 0; i <= resp.length - 1; i += 4) {
                        if (resp[i] && resp[i + 1] && resp[i + 2] && resp[i + 3]) {
                            if (resp[i][1] && resp[i + 1][1] && resp[i + 2][1] && resp[i + 3][1]) {
                                Movie.findOne({Title: resp[i][1], ReleaseYear: resp[i + 1][1], Format: resp[i + 2][1] ,Stars: resp[i + 3][1]}, function (err, success) {
                                    if (err) {
                                        ...
                                        if (success == null) {
                                            ...
                                            errorHandler(res, 'Error! Movie with this fields already existed!');
                                        }
                                    }
                                })} else {
...
module.exports.addMovie = async function (req, res) {
    if (req.body.Title && req.body.ReleaseYear && req.body.Format && req.body.Stars) {
        ...
        Movie.findOne({Title: req.body.Title, ReleaseYear: req.body.ReleaseYear, Format: req.body.Format, Stars: req.body.Stars}, function (err, success) {
            if (err) {
                errorHandler(res, err);
            } else {
                if (success == null) {
